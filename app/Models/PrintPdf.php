...
class PrintPdf extends Model
{
    ...
    public function printMotel($motel_id, $start_time, $end_time)
    {
...
<p>Tiền nước: 50.000đ/người thanh toán vào đầu các tháng.</p>
<p>Hợp đồng có giá trị kể từ ngày ' . Carbon::parse($start_time)->format('d') . ' tháng ' . Carbon::parse($start_time)->format('m') . ' ' . Carbon::parse($start_time)->format('Y') . ' đến ngày ' . Carbon::parse($end_time)->format('d') . ' tháng ' . Carbon::parse($end_time)->format('m') . ' năm ' . Carbon::parse($end_time)->format('Y') . '</p>
